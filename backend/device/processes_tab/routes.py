...
FOREGROUND_PACKAGE_RE = re.compile(r"u0\s+([A-Za-z0-9._]+)\/")


def get_foreground_package_id(device_id: str) -> Optional[str]:
    adb = device_manager.adb_manager
    if not adb:
        return None
    
    try:
        output = adb.execute_shell(
            device_id,
            "dumpsys activity activities 2>/dev/null | grep -E 'mResumedActivity|topResumedActivity' || true",
            timeout=5
        )
...
async def list_processes(device_id: str):
    try:
        ...
        foreground_package_id = get_foreground_package_id(device_id)
        
        return {
            "processes": processes,
            "count": len(processes),
            "foreground_package_id": foreground_package_id,
            "stats": {
                "total": len(processes),
