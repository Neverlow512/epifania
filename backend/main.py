...
from device.packages_tab.routes import router as packages_router
from device.workshop_tab.routes import router as workshop_router
from device.workshop_tab.instrumentation.instrumentation_routes import router as instrumentation_router
from device.workshop_tab.operations.cleanup import cleanup_old_sessions
import asyncio
...
async def startup_event():
    ...
    # Clean old operation logs (keep latest 10 per package)
    cleanup_old_sessions(keep_per_package=10)
    
    # Clean old temp states per package retention config
    from device.workshop_tab.storage.temp_state_cleanup import temp_state_cleanup
    temp_state_cleanup.cleanup_old_temp_states()
...
async def shutdown_event():
    ...
    # Clean up all Frida sessions before shutdown
    from device.workshop_tab.frida_session.session_manager import frida_session_manager
    logger.info("Cleaning up Frida sessions...")
    frida_session_manager.cleanup_all()
    logger.info("Frida sessions cleaned")
    
    from device.workshop_tab.operations.session_manager import session_manager
    logger.info("Generating workshop session summaries...")
    session_manager.generate_all_summaries()
    logger.info("Session summaries complete")
...
app.include_router(packages_router, prefix="/api/devices", tags=["packages"])
app.include_router(workshop_router, prefix="/api/devices", tags=["workshop"])
app.include_router(instrumentation_router, prefix="/api/devices", tags=["instrumentation"])
...
@app.websocket("/ws/devices/{device_id}/workshop/discovery")
async def websocket_workshop_discovery(websocket: WebSocket, device_id: str):
    from device.workshop_tab.discovery.discoverer import get_discovery_status
    
    await websocket.accept()
    logger.info(f"Workshop discovery WebSocket connected for {device_id}")
        ...
        while True:
            ...
            if status:
                ...
                if status.get("state") in ["complete", "error", "cancelled"]:
                    ...
                    logger.info(f"Workshop discovery {status.get('state')} for {device_id}, closing WebSocket")
                    break
            else:
                await websocket.send_json({"state": "idle", "progress": 0})
            
            await asyncio.sleep(0.3)
            
    except WebSocketDisconnect:
        logger.info(f"Workshop discovery WebSocket disconnected for {device_id}")
    except Exception as e:
        logger.error(f"Workshop discovery WebSocket error for {device_id}: {str(e)}")


@app.websocket("/ws/devices/{device_id}/workshop/operation/{operation_type}")
async def websocket_operation_progress(websocket: WebSocket, device_id: str, operation_type: str):
    from device.workshop_tab.operations.operation_manager import operation_manager
    
    await websocket.accept()
    logger.info(f"Workshop operation WebSocket connected for {device_id}/{operation_type}")
    ...
    try:
        while True:
            ...
            if progress:
                ...
                if progress.get("completed") or progress.get("cancelled"):
                    logger.info(f"Workshop operation {operation_type} finished for {device_id}")
                    break
            else:
        ...
        logger.info(f"Workshop operation WebSocket disconnected for {device_id}/{operation_type}")
    except Exception as e:
        logger.error(f"Workshop operation WebSocket error for {device_id}/{operation_type}: {str(e)}")


@app.websocket("/ws/devices/{device_id}/instrumentation/observer")
async def websocket_observer(websocket: WebSocket, device_id: str):
    from device.workshop_tab.instrumentation.tools.observer.hook_manager import observer_hook_manager
    
    await websocket.accept()
    logger.info(f"Observer WebSocket connected for {device_id}")
    
    try:
        while True:
            if not observer_hook_manager.is_session_active(device_id):
                await websocket.send_json({"type": "session_ended"})
                break
            
            status = observer_hook_manager.get_session_status(device_id)
            if status:
                top_hooks = observer_hook_manager.get_top_hooks(device_id, limit=50)
        ...
        logger.info(f"Observer WebSocket disconnected for {device_id}")
    except Exception as e:
        logger.error(f"Observer WebSocket error for {device_id}: {e}")
