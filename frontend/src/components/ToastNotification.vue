<template>
  <div class="toast-container">
    ...
    <!-- Guidance toasts (blue circles with exclamation mark) -->
    <transition-group name="toast" tag="div">
      <div
        v-for="toast in guidanceToasts"
        :key="'guidance-' + toast.id"
        class="toast-aggregate"
      >
        <button
          type="button"
          class="toast-bubble toast-guidance"
          @click="toggleExpanded(toast.id)"
        >
          <span class="toast-bubble-icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <transition name="toast-panel">
          <div
            v-if="toast.expanded"
            class="toast-panel toast-guidance"
          >
            <div class="toast-panel-content">
              <div class="toast-panel-text">
                <div class="toast-text">
...
export default {
  ...
  setup() {
    ...
    const regularToasts = computed(() =>
      toasts.value.filter(t => t.type !== 'error' && t.type !== 'guidance')
    )
    ...
    const guidanceToasts = computed(() =>
      toasts.value.filter(t => t.type === 'guidance')
    )
    ...
    const toggleExpanded = (id) => {
      const toast = [...errorToasts.value, ...guidanceToasts.value].find(t => t.id === id)
      if (toast) {
