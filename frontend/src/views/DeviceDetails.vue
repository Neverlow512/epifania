<template>
  <div class="container mx-auto p-6 max-w-[1800px]">
    <!-- Back Button -->
    <button 
      type="button"
    ...
    <div v-else>
        ...
        <div class="card-body p-4">
          <div class="flex items-center gap-3 flex-wrap">
            ...
            <div 
              ...
              :class="workshopSessionStatus === 'active' ? 'badge-success' : 'badge-warning'"
            >
              <span 
                class="w-1.5 h-1.5 rounded-full"
                :class="workshopSessionStatus === 'active' ? 'bg-green-400 animate-pulse' : 'bg-yellow-400'"
              ></span>
              {{ workshopSessionStatus === 'active' ? 'Session Active' : 'Session Locked' }}
            </div>
      ...
      <div class="mt-6">
        <component 
          ...
          @update:workshopSessionActive="workshopSessionActive = $event"
...
import { getWorkshopClientId } from './device/workshop/composables/workshopClientId'
...
export default {
  ...
  setup(props) {
    ...
    const workshopSessionActive = ref(false)
    
    const workshopClientId = getWorkshopClientId()
    const workshopLockInfo = ref({ locked: false, client_id: null, expires_in: 0 })
    
    let processCheckInterval = null
    let connectionCheckInterval = null
    let workshopLockCheckInterval = null
    ...
    const isWorkshopLockedByOther = computed(() => {
      if (!workshopLockInfo.value.locked) return false
      return workshopLockInfo.value.client_id !== workshopClientId
    })
    
    const isWorkshopOwnedByMe = computed(() => {
      return workshopLockInfo.value.locked && workshopLockInfo.value.client_id === workshopClientId
    })
    
    const workshopSessionStatus = computed(() => {
      if (isWorkshopOwnedByMe.value) return 'active'
      if (workshopSessionActive.value) return 'active'
      return 'locked'
    })
    ...
    const tabs = computed(() => [
      ...
      { 
        name: 'workshop', 
        label: 'Workshop',
        locked: isWorkshopLockedByOther.value
      }
    ])
    ...
    const checkWorkshopLockStatus = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/devices/${deviceId.value}/workshop/session/info`
        )
        workshopLockInfo.value = response.data
      } catch (err) {
        console.error('Failed to check workshop lock status:', err)
        workshopLockInfo.value = { locked: false, client_id: null, expires_in: 0 }
      }
    }
    ...
    const startConnectionTracking = () => {
      ...
      workshopLockCheckInterval = setInterval(async () => {
        await checkWorkshopLockStatus()
      }, 3000)
    }
    ...
    const stopConnectionTracking = () => {
      ...
      if (workshopLockCheckInterval) {
        clearInterval(workshopLockCheckInterval)
        workshopLockCheckInterval = null
      }
    }
