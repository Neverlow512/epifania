<template>
  <div class="space-y-2">
    ...
    <div v-if="io && available" class="bg-black/40 rounded-lg border border-slate-700/50 p-3">
    ...
    <div v-else class="bg-black/40 rounded-lg border border-slate-700/50 p-3">
      <div class="flex items-center gap-2 text-slate-500 text-xs">
        ...
        <div>
          <span class="block">I/O stats not available</span>
          <span class="text-[10px] text-slate-600">Requires root access to read /proc/[pid]/io</span>
        </div>
      </div>
    </div>
  </div>
